...
Feature: Credit management
  ...
  @LoggedIn
  Scenario Outline: verify tab input customer data
    ...
    Examples:
      | submenuText       | menuText   | validCustomerCode | rowIndex | textInput   | typeInput | btnGenerate | index | btnNewReport | btnBack | btnEditInputCustomerData    | emptyData | btnDeleteInputCustomerData    | deleteSuccess            | lessThan1   | tabInputCustomer    | duplicateMessage                |
      | Credit Management | Accounting | a567              | 0        | name1,name2 | Provider  | Generate in input customer data | 0     | New report   | Back    | Edit in input customer data | 0         | Delete in input customer data | Delete data successfully | less Than 1 | Input customer data | There are some duplicated items |
  ...
  Scenario Outline: select record draft -> approve
    ...
    Examples:
      ...
      | Credit Management | Accounting | a567              | 0        | Survey institute and information source | Index for accounting system | 帝国データ       | 12                   | Edit    | Credit score number | 10               | Trading condition category | 通常取引         | Zip code of HQ | 111-1111     | Address of HQ (all) | Tel. | 023-345-3212 | Fiscal closing month | 01月         | Business factors | Approve    | Save successfully | Duplicate    | Save draft   | Industry classification | Generate in input customer data | Input customer data | name1,name2 | Provider  |
...
#    Then In Credit Management page, System show first message <fiscal> <isRequired>
  ...
  Scenario Outline: verify approve full data
    ...
    Examples:
      ...
      | Credit Management | Accounting | a567              | 0        | Survey institute and information source | Index for accounting system | 帝国データ       | 12                   | Credit score number | 10               | Trading condition category | 通常取引         | Zip code of HQ | 111-1111     | Address of HQ (all) | Tel. | 023-345-3212 | Fiscal closing month | 01月         | Business factors | Approve    | Save successfully | Industry classification | New report   | Approve    | Input customer data | name1,name2 | Provider  | Generate in input customer data |
