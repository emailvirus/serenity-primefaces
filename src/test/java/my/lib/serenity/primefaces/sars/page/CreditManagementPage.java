...
public class CreditManagementPage extends AbstractPage {
    ...
    @FindBy(xpath = "//form[contains(@id,'formSearchCustomerByCode')]//input[contains(@id,'customer_code')]")
    WebElementFacade inputCustomerCodeElement;

    @FindBy(xpath = "//form[contains(@id,'formSearchCustomerByCode')]//button[contains(@id,'searchButtonCustomerCode')]")
    ...
    public boolean is_initialization_screen() {
        return this.getListRadioSearchType().get(0).isEnabled() && this.getListRadioSearchType().get(0).isSelected() && this.getListRadioSearchType().get(1).isEnabled()
                && this.getInputCustomerCode().isEnabled() && !this.getButtonSearchByCustomerCode().isEnabled();
    }

    public void search_customer_by_code(String code) {
        this.getListRadioSearchType().get(0).click_and_wait();
    ...
    public void fill_value_for_input_id_name_card(String idNameCard) {
        this.getListRadioSearchType().get(1).click_and_wait();
    ...
    public ButtonComponent getButtonSearchByCustomerCode() {
        if (this.buttonSearchByCustomerCode == null) {
            this.buttonSearchByCustomerCode = PFComponentFactory.wrapButton(this, this.buttonSearchByCustomerCodeElement);
        }
...
//        nameAndBranch.add(menuText.split("\n")[1].split(":")[1].trim());
    ...
    public void change_survey_date_to_next_day_of_selected_record() {
        if (StringUtils.isNotEmpty(this.dateOfSurveySelected)) {
            LocalDate date = CommonUtils.parseStringToYearMonthDayLocalDate(this.dateOfSurveySelected).plusDays(1);
            PFComponentFactory.wrapCalendarElement(this, this.findBy("//form[@id='surveySpecForm']//button[contains(@class,'ui-datepicker-trigger')]/ancestor::span")).select_date(date, Locale.ENGLISH);
        }
    }
