...
public class SearchHistoryByDatePage extends AbstractPage {
    ...
    public boolean is_initialization_screen() {
        return this.getCalendarDate().isCurrentEnable() && this.getDropdownBranch().isEnabled() && this.getAutoCompleteEmployee().isCurrentEnable()
                ...
                && this.getButtonShowAllBusinessDetails().isEnabled() && this.getToggleColumn().isEnabled() && this.getCheckboxRetired().isVisible();
    }
    ...
    public boolean should_show_required_branch_and_employee_error_message() {
        ...
        return isShowMessage && listErrorMessagesText.contains(CommonConstant.BRANCH_REQUIRED_MESSAGE_TEXT) && listErrorMessagesText.contains(CommonConstant.EMPLOYEE_REQUIRED_MESSAGE_TEXT);
    }
    ...
    public void select_row(int rowIndex) {
        this.getTableSearchByDateResult().select_row_by_index(rowIndex);
    }
